...
class App extends Component {
  ...
  componentDidMount() {
    fetch(`/auth/ext/user/${usernameFormServer}`)
      .then((res) => res.json())
      .then((data) => {
        let arr = [data];
    ...
    fetch(`/auth/ext/photo/${usernameFormServer}`)
      .then((res) => res.json())
      .then((photo) => {
    ...
    return (
      <div className="container" id="container">
        ...
        <div className="jumbotron" id="grid">
          {this.state.Nophoto == false
            ? this.state.photodata.map((result) => (
                <Photo key={result._id} data={result.photoPath} />
              ))
            : null}

          <br />
          {this.state.isUser === true
            ? this.state.userdata.map((results) => (
                <Results key={results._id} user={results} />
              ))
            : null}
          <br />
          <Uploadfile
...
class Results extends Component {
  render() {
    return (
      <div id="Results" className="text-right">
    ...
    return (
      <div>
        {this.props.photoStatus == true ? (
          <button class="btn-default" id="but">
            {" "}
            uploadimage
          </button>
        ) : null}
