...
router.post('/signup', ulrencodedParser,function(req , res){
  var name = req.body.name;
    ...
    req.checkBody('name','Name is required').notEmpty();
     ...
     req.checkBody('email','Email must between 10 and 100 characters long').len(10,100);

    req.checkBody('email','Email is not valid').isEmail();
    req.checkBody('password','password is required').notEmpty();
    req.checkBody('password','passwords must be atleast 8 long and contain at least one number').isLength({min:8}).matches(/\d/)
    req.checkBody('password','your password must contain atleast one lowercase and one Uppercase letter').matches(/[a-z]/).matches(/[A-Z]/)
    req.checkBody('confpassword','Passwords must match').equals(req.body.password);
    ...
    if(errors){
        ...
        res.redirect('/signup');
      ...
      else{  User.getUserByUsername(email,function(err, user){
            ...
            if(user){
                ...
                req.flash('error_msg','User already exits, if you have forgotten your password, return to home page and click on the forgotten password button ')
                res.redirect('/signup')
            }
...
router.post('/profile', ulrencodedParser,passport.authenticate('local', {successRedirect:'/profile',failureRedirect:'/',failureFlash:true}));
...
transporter.sendMail(mailOptions,function(error,info){
    ...
    req.flash('Success_msg',`Reset instructions  have been sent to your email, check your inbox in a few minutes`)
   res.redirect('/')
    }
})
    }
...
if(err) {
    ...
    User.deleteUser(username,function(err,done){
        ...
        req.flash('error_msg','Failed to delete account ;try again and retype in password')
      res.redirect('/contact')
                  ...
                  photo.Savephoto(username,photopath,(err,img)=>{
                      if(err) {
                          console.log(err)
                           ...
                           res.redirect('/profile')
                        log(`${res.locals.user.name} has added a new photoimage at ${img.photoPath}`)
                      }
                  })
              }
          })
