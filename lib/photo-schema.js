...
module.exports.Updatephoto = function(username,path,cb){
    photoapi.findOne({username:username},function(err,ismatch){
        if(err||ismatch==null) {
            ...
            // check for the os platform and  change dir paths corresponding to the platform requirements
             if (IsLinux==true) {
                 let mypath = `\public${ismatch.photoPath}`
             }
             
             
             else{
                  mypath = `/public${ismatch.photoPath}`
                }
...
module.exports.Deletephoto = function(username,cb){
 ...
 // check for the os platform and  change dir paths corresponding to the platform requirements
 if (IsLinux == true) {
     let imagepath = `\public${photo.photoPath}`
 } else {
